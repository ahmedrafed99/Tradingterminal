...
3. When the frontend is connected, the **tick aggregator** receives live quote ticks and builds candles in memory. A **boundary timer** (using the same `Math.ceil(Date.now() / periodMs) * periodMs` math as the chart's countdown label) fires **250ms after each candle closes** — independent of when the exchange sends its next tick. The `candle.close` at that moment is evaluated. When no frontend is connected, the **bar aggregator** falls back to REST polling aligned to candle boundaries (+ 3s buffer). When a new condition is armed, `reschedule()` also does an **immediate poll** for any timeframes whose candle closed within the last 10s, preventing a race where the reschedule cancels the pending poll timer
...
- **Dual-mode bar detection**: In **local mode** (frontend running), the frontend forwards live quote ticks to the backend over a dedicated WebSocket (`/ws/condition-quotes`). The backend's `tickAggregator` builds candles from ticks and uses **boundary timers** — `setTimeout` aligned to `Math.ceil(Date.now() / periodMs) * periodMs + 250ms`, the same clock math as the chart's bar countdown label — to evaluate `candle.close` ~250ms after each bar closes. This is independent of when the exchange sends its first new-period tick, eliminating the 1–3s delay that tick-transition detection caused. A dedup map prevents double-evaluation if both the boundary timer and a new-period tick arrive for the same candle. In **remote mode** (no frontend connected), the `barAggregator` falls back to REST polling aligned to candle close times with a 3s buffer. The bar aggregator automatically skips polling when a tick-forwarding frontend is connected.
