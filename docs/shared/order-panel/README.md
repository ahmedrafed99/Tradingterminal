...
- **Market-closed state**: `useMarketStatus()` (1 s reactive hook) returns `{ open, reopenLabel }` and drives `canPlace`. When closed, buttons are `disabled:opacity-50` and a full-width amber banner appears below (12px gap): warning icon + "Market closed — {reopenLabel}", styled with `text-(--color-warning)` on a subtle `color-mix(in srgb, var(--color-warning) 8%, transparent)` background. The reopen label is dynamic: "reopens today 18:00 ET" during Mon–Thu maintenance or Sunday pre-open, "reopens Sun 18:00 ET" during the weekend. A second `isFuturesMarketOpen()` call inside `handlePlace` guards against the race where the hook value is stale at click time.
