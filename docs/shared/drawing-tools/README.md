...
3. **StrokePopover** (140px wide, centered): combined width + style picker. Top section: 4 width rows (1–4px) with SVG line previews. Horizontal divider. Bottom section: 3 style rows (Solid, Dashed, Dotted) with SVG dash previews. Clicking a width row changes only `strokeWidth`; clicking a style row changes only `lineStyle`. Active selection highlighted with `--color-table-stripe` background + `--color-warning` (orange) text + orange SVG stroke. Hover uses `--color-hover-row` with `transition-colors`. All SVG line previews use `shapeRendering="crispEdges"` + `strokeWidth="1"` (style rows) to match the visual weight of 1px CSS borders — without `crispEdges`, SVG antialiasing blurs lines across 2 physical pixels making them appear thicker than they are.
4. **TemplatePopover** (220px wide, hline only): saved style templates with export/import
5. **RectSettingsPopover** (294px wide, rect only): extend mode (None / Extend right / Extend left / Both) via checkbox dropdown, and middle line toggle (checkbox + color swatch + line style dropdown). Line style dropdown SVG preview uses `shapeRendering="crispEdges"` + `strokeWidth="1"` for the same reason as StrokePopover.
