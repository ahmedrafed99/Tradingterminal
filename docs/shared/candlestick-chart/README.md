...
- **Session-only mode (gap collapse)**: A **Session** toggle in `ChartToolbar` hides market-closed periods (CME overnight 17:00–18:00 ET, weekends, holidays) so every candle renders directly adjacent to the previous one — no blank whitespace. Implementation:
