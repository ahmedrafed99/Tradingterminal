...
import { FONT_SIZE, RADIUS, SHADOW } from '../../constants/layout';
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: `${navTopPad} 20px 8px` }}>
        <button
          onClick={prevMonth}
          className="flex items-center justify-center text-(--color-text-muted) hover:text-(--color-text) transition-colors"
          style={{ width: 24, height: 24, background: 'transparent', border: 'none', cursor: 'pointer', borderRadius: RADIUS.MD }}
        >
          <ChevronLeft />
        </button>
            ...
            if (mode === 'single') {
              ...
              return (
                <button
                  ...
                  className="hover:bg-(--color-hover-row) transition-colors"
                  style={{
              ...
              return (
                <button
                  ...
                  disabled={isDisabled}
                  className={isDisabled ? '' : 'hover:bg-(--color-hover-row) transition-colors'}
                  style={{
                    fontSize: FONT_SIZE.OVERLAY, textAlign: 'center', padding: '6px 0',
                    borderRadius: RADIUS.MD,
      ...
      {mode === 'single' ? (
        <div style={{ borderTop: '1px solid var(--color-border)', display: 'flex', justifyContent: 'flex-end', gap: 8, padding: '12px 20px' }}>
          <button
            onClick={onClose}
            className="text-(--color-text) hover:bg-(--color-hover-row) transition-colors"
            style={{
