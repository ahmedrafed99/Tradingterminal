...
- **Left section**: Instrument selector | divider | pinned timeframe buttons | timeframe dropdown
  ...
  - Dropdown lists all timeframes grouped by section (Seconds: 5s, 15s — Minutes: 1m, 3m, 15m — Hours: 1h, 4h — Days: D) with a star toggle to pin/unpin; custom timeframes fall into their matching section; duplicate detection blocks adding an existing timeframe
...
- **Countdown avoidance**: `setCountdownPrice(price)` is fed from `useChartBars` on every quote tick. In `priceAxisViews()`, all axis labels (hlines + order lines registered via `setCoordinator()`) are passed through `stackAxisLabels()`. Labels within `countdownZone = COUNTDOWN_BADGE_HALF_H + PRICE_AXIS_LABEL_H / 2` px of the countdown's Y are pushed outward (above if the label was above, below otherwise), giving a zero-pixel edge-to-edge gap between any pushed label and the countdown badge. After the push, labels are stacked with a minimum separation of `PRICE_AXIS_LABEL_H`. A second re-clamp pass runs after stacking to fix a forward-stacking edge case: the stacking loop pushes items downward, which can drag an above-zone label back into the badge zone — the re-clamp corrects only above-zone items. Because `countdownZone` (28 px) exceeds `labelHeight` (20 px), the re-clamp can collapse multiple above-zone labels to the same Y; a final reverse stacking pass then spreads them upward from the zone edge.
- **Selected hline canvas label**: When a drawing is selected, its axis label is painted via `SelectedDrawingAxisRenderer` on the canvas layer. Its height is derived from `PRICE_AXIS_LABEL_H` (same constant used in the zone calculation) so the edge-to-edge gap is exactly zero when pushed to the zone boundary.
...
- **LWC native label permanently hidden**: `horzLine.labelVisible: false` is set in `CHART_OPTIONS` (`chartTheme.ts`). LWC's built-in canvas crosshair label on the price scale is disabled at chart creation — the HTML `CrosshairLabelPrimitive` is the sole crosshair price label. This prevents two overlapping labels appearing simultaneously on the price axis.
- **Suppressed during hline drag**: `suppress(true)` hides the label and blocks `updateCrosshairPrice()` calls. The HTML overlay updates instantly while the canvas drawing label lags one frame — suppressing the crosshair label eliminates the flicker and prevents any label from peeking through when de-overlap pushes the drawing label away from another drawing. Restored on mouseup.
...
- **Timeframe-switch position preservation**: When switching timeframes on the same instrument, the chart preserves both the vertical (price scale) and horizontal (scroll) position set by the user. Before loading new bars, `useChartBars` snapshots `timeScale().scrollPosition()` (bars from the last bar to the right edge) and skips the `autoScale: true` reset that is only needed for new instruments. After `series.setData()`, `scrollToPosition(savedScrollPos, false)` restores horizontal position; vertical stays fixed automatically because `autoScale` remained `false`. New-instrument loads retain the existing reset behavior (autoScale → default 100-bar range → autoScale off). Drill-down (right-click → zoom into candle) overrides both cases and always sets the target logical range.
