...
- **Countdown avoidance**: `setCountdownPrice(price)` is fed from `useChartBars` on every quote tick. In `priceAxisViews()`, all axis labels (hlines + order lines registered via `setCoordinator()`) are passed through `stackAxisLabels()`. Labels within `countdownZone = COUNTDOWN_BADGE_HALF_H + PRICE_AXIS_LABEL_H / 2` px of the countdown's Y are pushed outward (above if the label was above, below otherwise), giving a zero-pixel edge-to-edge gap between any pushed label and the countdown badge. After the push, labels are stacked with a minimum separation of `PRICE_AXIS_LABEL_H`. A second re-clamp pass runs after stacking to fix asymmetric forward-stacking: the stacking loop pushes items downward, which can drag an above-zone label 1 px back into the badge — the re-clamp pass corrects only above-zone items.
