...
- **Hide drawings**: When any trades are visible on the chart (`visibleTradeIds.length > 0`), a "Hide drawings" button appears in the header bar next to the date preset selector. Clicking it calls `clearVisibleTradeIds()` to dismiss all trade zone overlays at once. The button disappears when no trades are highlighted.
